...
module('Unit | Component | form-adapters/base', function (hooks) {
  ...
  hooks.beforeEach(function () {
    this.component = this.owner
      .factoryFor('component:fit-form')
      .create({ adapter: 'base' });
  });
  ...
  module('without models', function (hooks) {
    ...
    test('the default state', function (assert) {
      ...
      assert.deepEqual(
        this.form.models,
        [],
        'the form models are an empty array'
      );
    ...
    module('submit the form', function (hooks) {
      ...
      test('while submitting', function (assert) {
        ...
        assert.equal(
          onsubmitArgs.length,
          1,
          'onsubmit called with one argument'
        );
        assert.equal(
          onsubmitArgs[0],
          this.form,
          "onsubmit called with the 'form' as the first argument"
        );
      });
      ...
      test('submitting the form is fulfilled', async function (assert) {
        ...
        assert.equal(
          onsuccessArgs.length,
          2,
          'onsuccess called with two arguments'
        );
        assert.equal(
          onsuccessArgs[0],
          42,
          'onsuccess called with the submit result as the first argument'
        );
        assert.equal(
          onsuccessArgs[1],
          this.form,
          "onsuccess called with the 'form' as the second argument"
        );
      });
      ...
      test('submitting the form is rejected', async function (assert) {
        ...
        assert.equal(
          onerrorArgs.length,
          2,
          'onerror called with two arguments'
        );
        assert.equal(
          onerrorArgs[0],
          'error',
          'onerror called with the submit error as the first argument'
        );
        assert.equal(
          onerrorArgs[1],
          this.form,
          "onerror called with the 'form' as the second argument"
        );
      });
    });
    ...
    test('cancel the form', async function (assert) {
      ...
      const oncancel = sinon
        .stub(this.form, 'oncancel')
      ...
      assert.equal(
        oncancelArgs[0],
        this.form,
        "oncancel called with the 'form' as the first argument"
      );
    ...
    module('validate the form', function (hooks) {
      hooks.beforeEach(function () {
        this.deferred = defer();
        this.onvalidate = sinon
          .stub(this.form, 'onvalidate')
          .returns(this.deferred.promise);
      ...
      test('while validating', function (assert) {
        ...
        assert.equal(
          onvalidateArgs.length,
          1,
          'onvalidate called with one argument'
        );
        assert.equal(
          onvalidateArgs[0],
          this.form,
          "onvalidate called with the 'form' as the first argument"
        );
      });
      ...
      test('validating the form is fulfilled with false', async function (assert) {
        ...
        assert.equal(
          oninvalidArgs[0],
          undefined,
          'oninvalid called with undefined as the first argument'
        );
        assert.equal(
          oninvalidArgs[1],
          this.form,
          "oninvalid called with the 'form' as the second argument"
        );
      });
      ...
      test('validating the form is rejected', async function (assert) {
        ...
        const oninvalidArgs = this.oninvalid.getCall(0).args;
        assert.equal(
          oninvalidArgs.length,
          2,
          'oninvalid called with two arguments'
        );
        assert.equal(
          oninvalidArgs[0],
          'invalid',
          'oninvalid called with the validate invalidation as the first argument'
        );
        assert.equal(
          oninvalidArgs[1],
          this.form,
