...
export default class FitFormService extends Service {
  /**
  ...
  constructor() {
    ...
    owner.registerOptionsForType('ember-fit-form@form-adapter', {
      instantiate: false,
    });
  ...
  lookupAdapter(name) {
    ...
    const availableAdapter = getOwner(this).lookup(
      `ember-fit-form@form-adapter:${dasherizedAdapterName}`
    );
    const localAdapter = getOwner(this).lookup(
      `form-adapter:${dasherizedAdapterName}`
    );
